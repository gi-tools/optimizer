...
type SlotKey = "flower" | "plume" | "sands" | "goblet" | "circlet"`,F=`interface IWeapon {
  ...
  = ${["hp","hp_","atk","atk_","def","def_","eleMas","enerRech_","heal_","critRate_","critDMG_","physical_dmg_","anemo_dmg_","geo_dmg_","electro_dmg_","hydro_dmg_","pyro_dmg_","cryo_dmg_","dendro_dmg_"].map(y=>`"${y}" //${n(y)}${B(y)}`).join(`
  ...
  = ${[...new Set(M)].sort().map(K=>`"${K}" //${n(`charNames_gen:${P(c.chars.LocationToCharacterKey(K),r)}`)}`).join(`
  ...
  = ${[...new Set(j)].sort().map(r=>`"${r}" //${n(`weaponNames_gen:${r}`)}`).join(`
